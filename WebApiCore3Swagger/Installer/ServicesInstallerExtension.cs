...
namespace WebApiCore3Swagger.Installer
{
    public static class ServicesInstallerExtension
    {
        ...
        {
            var servicesInstallers = typeof(Program).Assembly.ExportedTypes.Where(t => typeof(IServiceInstaller).IsAssignableFrom(t) && !t.IsInterface && !t.IsAbstract).Select(Activator.CreateInstance).Cast<IServiceInstaller>().ToList();
